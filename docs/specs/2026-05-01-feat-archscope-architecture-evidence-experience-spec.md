...
- Partial evidence must be explicit. A command must not print a full-success message if required artifacts were skipped or failed.
...
| SA3  | The first planning slice of the default evidence workflow writes the non-visual pack: `.diagram/manifest.json`, `.diagram/brief.md`, `.diagram/agent-context.json`, and `.diagram/architecture.mmd`; `report.html` may be marked `deferred` until the UI spec is complete. | Fixture execution and file existence/status assertions.                     |
...
- Should `scan` run `validate` by default, or only include validation when `.architecture.yml` exists?
...
The first planning slice should not attempt the whole product repositioning at once. Start with the artifact contract and default evidence workflow:

- implement `archscope scan .` as the selected default evidence workflow;
- define `ArchitectureEvidencePack` and `EvidenceManifest`;
- generate `brief.md`, `agent-context.json`, `architecture.mmd`, and `manifest.json` from existing analysis capabilities;
