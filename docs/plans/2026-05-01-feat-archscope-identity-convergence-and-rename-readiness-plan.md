...
- `package.json` still has description text `Generate architecture diagrams
from codebases`, while package name and repository URLs intentionally remain
  compatibility surfaces.
- `src/diagram.js` help describes the CLI as `Inspect architecture, governance,
and diagram workflows from code`; this may need sharper architecture-evidence
  language.
- `src/commands/init.js` still generates a CI sample headed
  `diagram-cli`, installs the `diagram CLI`, invokes `diagram`, and suggests
  `diagram init` / `diagram validate`.
- `.github/workflows/pr-impact-comment.yml` still emits
  `Generated by diagram-cli`.
- `.github/ISSUE_TEMPLATE/feature.yml` still asks how the `diagram-cli`
  command/format/rule behavior should work.
- `.github/ISSUE_TEMPLATE/issue.yml` mentions the failing `diagram` command
  without compatibility context.
...
- [ ] AC1: Identity audit exists and is classified.
- [ ] AC2: Historical docs are not rewritten as active product docs.
...
- [ ] AC8: Generated human output says Archscope where branding appears.
- [ ] AC9: Machine-output tests still pass.
- [ ] AC10: Rename-readiness artifact exists.
- [ ] AC11: Package/repo rename remain blocked pending dedicated specs.
- [ ] AC12: `.diagram` and `.diagramrc` remain default compatibility paths.
...
| ---- | ------- | ------------------ | ----------------------------------------- |
| P0   | pending | implementing-agent | Identity audit not started.               |
