...
- Resolve phase review steps through repo-local `.codex/skills/` first. If no repo-local equivalent exists, use the configured skill handles (`$simplify`, `$he-code-review`) from the active agent-skills runtime. If those external skills cannot be loaded or executed in the active environment, record them as unavailable coverage gaps and run a findings-first manual simplify/code-review checklist against the phase diff; do not mark the external skills as passed.
...
- [x] **P1 / Unit 2: Non-Visual Evidence Pack Writers**
...
- [x] **P2 / Unit 3: PR-Aware Scan Composition**
...
- [x] **P3 / Unit 4: Documentation and CI Evidence Surfacing**
...
| `.diagram/report.html`              | required as `written` when report generation succeeds; `failed` with brief fallback on write failure | required as `written` when report generation succeeds; `failed` with brief fallback on write failure                                        |
...
- [x] **P4 / Unit 5: Companion UI Spec for report.html**
...
- [x] **P5 / Unit 6: HTML Report Implementation**
...
- [x] **AC1:** `archscope scan .` exists as a documented command and does not regress existing commands.
- [x] **AC2:** `.diagram/manifest.json` indexes top-level and subordinate artifacts, starts the agent read order, records artifact statuses, avoids absolute paths, and handles deterministic timestamp/list behavior.
- [x] **AC3:** `scan` writes the non-visual pack: `.diagram/brief.md`, `.diagram/agent-context.json`, `.diagram/architecture.mmd`, and `.diagram/manifest.json`.
- [x] **AC4:** `agent-context.json` has stable schema versioning, deterministic support, compact summary fields, artifact pointers, and documented read order.
- [x] **AC5:** Console output prints a concise next-step summary naming `.diagram/manifest.json`, the current `primaryHumanArtifact`, the current `primaryAgentArtifact`, and exactly one stable next action.
- [x] **AC6:** `scan --format json --deterministic` emits canonical machine-envelope fields with parser-safe stdout, stable error categories, and `data.outcome` set to `success`, `partial`, or `failed`.
- [x] **AC7:** Partial evidence records artifact-level statuses and never reports full success or exits `0` when artifacts required for the current phase and mode fail or are unexpectedly deferred.
- [x] **AC8:** `scan . --base <ref> --head <ref>` reuses `workflow pr`, writes PR artifacts, preserves the `workflow pr` Architecture Review Brief field contract, and includes risk, blast radius, confidence, evidence, and reviewer checks.
- [x] **AC9:** README, CLI reference, getting-started, and CI artifact guidance present architecture evidence first and keep governance/migration detail in maintainer paths.
- [x] **AC10:** `generate`, `generate-all`, `validate`, `workflow pr`, `context`, `diagram`, and `@brainwav/diagram` compatibility behavior remain non-breaking.
- [x] **AC11:** Companion UI spec exists and maps report visual acceptance to SA6 and SA15.
- [x] **AC12:** `report.html` supports scan and PR modes only after AC11 is satisfied and report invocation policy is frozen.
- [x] **AC13:** Implementation reuses existing analysis, generation, context, PR-impact, machine-envelope, and manifest capabilities rather than creating a parallel pipeline; parity tests must prove shared manifest behavior and reused PR/generation outputs.
- [x] **AC14:** `npm run ci:artifacts` passes and proves CI can publish or expose the required repository-scan and PR-scan artifact paths and statuses from the CI artifact contract.
...
| P1       | complete | 2026-05-01: added non-visual scan artifact writers for `brief.md`, `agent-context.json`, and `architecture.mmd`; added shared evidence summary helper and `src/schema/agent-context-v1.schema.json`; final manifest now reflects actual non-visual writer status while keeping `report.html` deferred; partial writer failure records stable `artifact_write_failed` category and exits non-zero. Validation: `npm test -- test/scan-evidence-pack.test.js test/agent-context-contract.test.js test/scan-error-categories.test.js test/scan-manifest.test.js` pass (6 passing); `npm test -- test/scan-command.test.js test/scan-evidence-pack.test.js test/agent-context-contract.test.js test/scan-error-categories.test.js test/scan-manifest.test.js` pass (8 passing); `npm test -- test/scan-command.test.js test/evidence-manifest-parity.test.js test/generate-output-json.test.js test/machine-command-coverage.test.js test/json-capability-discovery.test.js` pass (12 passing); `node scripts/validate-machine-contracts.js` pass (`commandCount: 11`, includes `scan`); `npm test` pass (166 passing). Review gates: `$simplify` pass after consolidating writer failure handling, heading binding, shared summary counting, and test-only failure removal; `$he-code-review` manual fallback pass; artifact `artifacts/reviews/archscope-evidence-p1-manual-review.md`. | P1-scoped AC3, AC4, AC7, and AC13 evidence recorded; PR-specific scan composition remains P2 and docs/CI remain P3. |
| P2       | complete | 2026-05-01: added PR-aware scan composition by invoking the existing `workflow pr` JSON contract when comparison refs are supplied; scan now writes and indexes `pr-impact/pr-impact.json`, includes PR status in `data.pr`, adds PR mode/risk/reviewer-check fields to `brief.md` and `agent-context.json`, and preserves repository evidence with `pr_refs_unavailable` when refs cannot resolve. Validation: `npm test -- test/scan-pr-evidence.test.js test/pr-impact.test.js test/pr-impact-delta.test.js` pass (32 passing); `npm test -- test/scan-command.test.js test/scan-evidence-pack.test.js test/agent-context-contract.test.js test/scan-error-categories.test.js test/scan-manifest.test.js` pass (8 passing). Review gates: `$simplify` and `$he-code-review` findings fixed for PR base auto-detection, `data.pr` contract, PR brief mode/fields, unused PR payload return data, duplicate risk, and custom output-dir PR path consistency; artifact `artifacts/reviews/archscope-evidence-p2-manual-review.md`.                                                                                                                                                                                                                                                                                                                                                    | P2-scoped AC8 and remaining AC13 evidence recorded; docs/CI remain P3 and UI/report planning remain P4/P5.          |
| P3       | complete | 2026-05-01: updated README, getting-started, CLI reference, and architecture-testing docs to present `archscope scan .` as the default architecture evidence workflow; documented PR evidence via `scan --base --head`, canonical agent read order, scan machine output, and deferred `report.html`; replaced the shell-only `ci:artifacts` script with `scripts/assert-ci-artifacts.js` to assert repository-scan and PR-scan artifact paths/statuses plus validation JUnit output. Validation: `npm run ci:artifacts` pass (`ci artifact assertions: OK`); `npm run docs:style:changed` pass (`0 errors, 0 warnings and 0 suggestions in 4 files`); `npm test` pass (169 passing); `bash scripts/verify-work.sh --fast` pass with optional Local Memory warning only; `npm run ci:artifacts` pass after simplify cleanup. Review gates: `$simplify` found duplicated artifact assertions and the fix was applied; `$he-code-review` external/subagent attempt was unavailable because one reviewer only acknowledged instructions and the retry hung, so manual fallback review passed with no P0/P1/P2 findings; artifact `artifacts/reviews/archscope-evidence-p3-manual-review.md`.                                                                                                                                                                                              | P3-scoped AC9, AC10, AC13, and AC14 evidence recorded; UI/report planning remains P4/P5.                            |
| P4       | complete | 2026-05-01: created `docs/ui-specs/2026-05-01-archscope-evidence-report-ui-spec.md` as the companion UI spec required before `report.html` can be marked complete. The spec freezes default report generation for `archscope scan`, report failure fallback behavior, repository and PR report IA, responsive/accessibility requirements, artifact/manifest status behavior, and VAC1-VAC9 mapped to SA6 and SA15. Validation: `npm run docs:style:changed` pass. Review gates: `$simplify`/`$he-code-review` external attempts returned instruction acknowledgments only, so manual fallback phase review passed with no P0/P1/P2 findings; artifact `artifacts/reviews/archscope-evidence-p4-manual-review.md`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | P4-scoped AC11 evidence recorded; P5 can now implement `report.html` against the UI spec.                           |
| P5       | complete | 2026-05-01: implemented `src/renderers/report-html.js` and wired `archscope scan` to write `.diagram/report.html` by default after P4 froze the report UI contract; manifest now promotes `report.html` as `primaryHumanArtifact` when report generation succeeds, falls back to `brief.md` with `write_failure` status when report writing fails, and refreshes `agent-context.json` after report failure so agents see the final artifact status. Updated CI artifact assertions, scan tests, and docs to treat report as a written scan artifact. Validation: `npm test -- test/scan-report-html.test.js` pass (3 passing); `npm test -- test/scan-error-categories.test.js test/scan-report-html.test.js` pass (4 passing); focused scan suite pass (13 passing); `npm run docs:style:changed` pass; `npm run ci:artifacts` pass (`ci artifact assertions: OK`); `npm test` pass (172 passing); `bash scripts/verify-work.sh --fast` pass with optional Local Memory warning only; `npm run test:deep` pass (`deep-regression: OK`). Review gates: `$simplify`/`$he-code-review` external attempts returned instruction acknowledgments only; manual review found and fixed stale agent-context report-failure status before passing with no remaining P0/P1/P2 findings; artifact `artifacts/reviews/archscope-evidence-p5-manual-review.md`.                                    | P5-scoped AC12 and remaining AC13/AC14 evidence recorded.                                                           |
| Closeout | complete | 2026-05-01: final reliability review completed with no P0/P1/P2 findings. External reliability reviewer stalled without findings and was stopped, so manual fallback review recorded residual risks and validation evidence. Artifact: `artifacts/reviews/archscope-evidence-final-reliability-review.md`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | P0-P5 complete; final reliability-readiness gate passed.                                                            |

## Completed Work Handoff

P0-P5 are complete. The default evidence workflow now writes:
...
When `--base` or `--head` is supplied and refs resolve, scan also writes `.diagram/pr-impact/pr-impact.json`.

Operational notes:

- Keep `manifest.json` first in agent read order.
- Keep `.diagram/report.html` as `primaryHumanArtifact` only when report generation succeeds.
- Preserve `.diagram/brief.md` fallback when report writing fails.
- Re-run `npm run ci:artifacts`, `npm test`, `bash scripts/verify-work.sh --fast`, and `npm run test:deep` when changing scan artifact behavior.
