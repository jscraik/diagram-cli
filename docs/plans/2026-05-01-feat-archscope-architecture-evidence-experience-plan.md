...
- Whether `report.html` is generated by default in every scan or only when a `--report`/profile option is supplied.
...
- Run `$simplify` and `$he-code-review` after each completed implementation phase when this plan enters work mode.
...
- Modify: `src/commands/generate-all.js` only if shared manifest helpers need to be extracted from existing behavior
...
- Implement a minimal evidence-pack coordinator that can write `manifest.json` with expected top-level artifact entries and statuses.
...
- Focused: `npm test -- test/scan-evidence-pack.test.js test/agent-context-contract.test.js`
- Baseline: `npm test`
...
- Make `archscope scan .` and `archscope workflow pr . --base origin/main --head HEAD` the obvious workflows.
...
- [ ] **AC5:** Console output prints a concise next-step summary naming primary human and agent artifacts.
...
- [ ] **AC12:** `report.html` supports scan and PR modes only after AC11 is satisfied.
- [ ] **AC13:** Implementation reuses existing analysis, generation, context, PR-impact, machine-envelope, and manifest capabilities rather than creating a parallel pipeline.
...
| ------------------------------------------------ | ------------------------------------------------------ | ---------------------------------------------------------------------------- |
| `scan` duplicates existing command logic         | Long-term drift and inconsistent artifacts             | Reuse shared internals; code review against SA23/AC13 after each phase.      |
