...
| --- | --- | --- | --- |
| P0 | complete | Codex | 2026-05-02: sharpened README, getting-started, and CLI reference so the first-read path leads with PR review, `archscope scan .`, and agent handoff before media commands. Validation: focused docs grep, Vale, `bash scripts/verify-work.sh --fast`, and `git diff --check` passed. Review artifact: `artifacts/reviews/archscope-product-sharpness-p0-manual-review.md`. |
| P1 | complete | Codex | 2026-05-02: documented deterministic repo and PR scan recipes, including `archscope scan . --base origin/main --head HEAD --format json --deterministic`, while keeping agent-specific aliases deferred. Validation: recipe grep, premature alias guard using `rg -n "archscope agent" ... \|\| true`, Vale, `bash scripts/verify-work.sh --fast`, and `git diff --check` passed. Review artifact: `artifacts/reviews/archscope-product-sharpness-p1-manual-review.md`. |
| P2 | complete | Codex | 2026-05-02: preserved partial scan exit `1` for compatibility while documenting `data.outcome: "partial"` plus `errors[].category` as the agent-safe discriminator. Validation: scan category and machine-output focused tests, Vale, and `git diff --check` passed. Review artifact: `artifacts/reviews/archscope-product-sharpness-p2-manual-review.md`. |
| P3 | complete | Codex | 2026-05-02: enriched text-mode `archscope scan` summaries with pack status, component count, primary artifacts, warnings, and PR review focus. Validation: `npm test -- test/scan-command.test.js test/scan-pr-evidence.test.js test/scan-evidence-pack.test.js` passed. Review artifact: `artifacts/reviews/archscope-product-sharpness-p3-manual-review.md`. |
| P4 | complete | Codex | 2026-05-02: kept `generate-video` and `generate-animated` available while labeling them optional advanced media across docs and unknown-command guidance. Validation: media wording grep for `generate-video\|generate-animated\|advanced\|optional\|media`, generated-output identity tests, command identity tests, and Vale passed. Review artifact: `artifacts/reviews/archscope-product-sharpness-p4-manual-review.md`. |
| P5 | complete | Codex | 2026-05-02: completed compatibility and closeout validation without additional source changes. Validation: focused identity/scan tests, `npm test`, `npm run test:deep`, `npm run ci:artifacts`, `npm run migration:readiness`, and `bash scripts/verify-work.sh --fast` passed. Review artifact: `artifacts/reviews/archscope-product-sharpness-p5-manual-review.md`. |
